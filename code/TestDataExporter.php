...
class TestDataExporter extends Controller
{
     ...
     */
    public function objectPresent($object, $buckets, $queue = null)
    {
        // Check buckets
     ...
     */
    public function traverseRelations($object, $buckets, &$queue)
    {
     ...
     */
    public function generateYML($object, $buckets)
    {
        $output = '';
        ...
        $dbField = $object->dbObject($field);
            if (!is_object($dbField) || !($dbField instanceof DBField)) {
                continue;
            }
     ...
     */
    public function assureHasTag($object)
    {
        if (!$object->YMLTag) {
            $object->YMLTag = $this->getTag($object);
        }
    }
     ...
     */
    public function generateIDs($textual, $class)
    {
        $where = array();
     ...
     */
    public function extractParams($content)
    {
     ...
     */
    public function export($data, $form)
    {
